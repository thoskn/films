...
def augment(df: pd.DataFrame, links_csv_path: str, ratings_csv_path: str) -> pd.DataFrame:
    df = join_with_ids(df, links_csv_path)
    df = join_with_ratings(df, ratings_csv_path)
    return df
